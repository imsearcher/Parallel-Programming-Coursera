...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
      var m= GenMap[Point, GenSeq[Point]]();
      means.map {x => m+=x->GenSeq();}
      m++points.groupBy {x => findClosest(x,means)};
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map {  
      x => findAverage(x,classified.get(x).getOrElse(IndexedSeq()));};
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val dif=oldMeans.map { x => 
        val i=oldMeans.indexOf(x);
        x.squareDistance(newMeans(i));}
    dif.forall { x => x<=eta }
  }

  @tailrec
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val newMeans=update(classify(points.par, means.par),means)
