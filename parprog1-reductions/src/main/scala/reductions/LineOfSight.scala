...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(end-from<=threshold){
      val angle=upsweepSequential(input,from,end);
      new Leaf(from,end,angle);
    }else{
      val mid=(from+end)/2;
      val (r1,r2)=parallel(upsweep(input,from,mid,threshold),
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match{
      ...
      case Node(l,r)=>{
        val m=if(startingAngle<l.maxPrevious) l.maxPrevious else startingAngle;
        parallel(downsweep(input,output,m,l),
